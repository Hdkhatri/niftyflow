...
# instruments_df = None # we will load this in the live_trading function to ensure it's fresh for each thread
...
def get_token_for_symbol(symbol,instruments_df):
    ...
    if not row.empty:
        print(f"✅ Found token for {symbol}: {row['instrument_token'].values[0]}")
...
def get_option_ltp(tradingsymbol, user):
    exchange = get_exchange(tradingsymbol)
    return get_last_price(f"{exchange}:{tradingsymbol}", user)
