...
import sqlite3
import logging
from crud import get_stoploss_by_trade_config, get_tradable_symbols
...
#     exit(1)
# logging.info(f"ℹ️ {SERVER} | Instrument token for {SYMBOL}: {instrument_token} at current time {current_time}")
...
def log_instruments_file_mod_time(filename):
    ...
    try:
        ...
        print(f"ℹ️ {SERVER} | {filename} last modified: {mod_time}")
        logging.info(f"ℹ️ {SERVER} | {filename} last modified: {mod_time}")
    except Exception as e:
        print(f"❌ {SERVER} | Failed to get modification time of {filename}: {e}")
        logging.error(f"❌ {SERVER} | Failed to get modification time of {filename}: {e}")
...
def save_trade_config(new_config):
    ...
    try:
        ...
        print(f"❌{SERVER} | {new_config.get('KEY')} Error saving trade config: {e}")
        logging.error(f"❌{SERVER} | {new_config.get('KEY')} Error saving trade config: {e}")
...
def get_trade_configs(user_id):
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("""
            SELECT USER_ID, KEY,TRADABLE_MAIN_SYMBOL_PK,TRADE_BASE_ON, STRATEGY, INTERVAL, LOT, NEAREST_LTP, INTRADAY, NEW_TRADE,
        ...
        for row in rows:
            (
                USER_ID, KEY, TRADABLE_MAIN_SYMBOL_PK, TRADE_BASE_ON, STRATEGY, INTERVAL, LOT, NEAREST_LTP, INTRADAY, NEW_TRADE,
                REAL_TRADE, EXPIRY, HEDGE_TYPE, HEDGE_ROLLOVER_TYPE, MONTHLY_STOPLOSS, ACTIVATE_MONTHLY_SL, STOPLOSS_PER_TRADE, ACTIVATE_SL_PER_TRADE, THREAD_STATUS, LAST_ERROR_REASON, ERROR_TIMESTAMP
            ) = row

            config_dict = {
                "INTERVAL": INTERVAL,
                "TRADABLE_MAIN_SYMBOL_PK": TRADABLE_MAIN_SYMBOL_PK,
                "TRADE_BASE_ON": TRADE_BASE_ON,
                "LOT": LOT,
            ...
            tradable_symbol = get_tradable_symbols(config_dict['TRADABLE_MAIN_SYMBOL_PK'])
            print(f"Fetched tradable symbol for {config_dict['TRADABLE_MAIN_SYMBOL_PK']}: {tradable_symbol}")
            config_dict['TRADABLE_SYMBOL'] = tradable_symbol
...
def get_keywise_trade_config(key):
    try:
        ...
        cursor.execute("""
			...
			GROUP by USER_ID, KEY, TRADABLE_MAIN_SYMBOL_PK, TRADE_BASE_ON, STRATEGY, INTERVAL, LOT, NEAREST_LTP, INTRADAY, NEW_TRADE,
                REAL_TRADE, EXPIRY, HEDGE_TYPE, HEDGE_ROLLOVER_TYPE, MONTHLY_STOPLOSS, ACTIVATE_MONTHLY_SL, STOPLOSS_PER_TRADE, ACTIVATE_SL_PER_TRADE
        """, (key,))
        ...
        print(f"❌{SERVER} | {key} | Error fetching trade configs: {e}")
        logging.error(f"❌{SERVER} | {key} | Error fetching trade configs: {e}")
        send_telegram_message_admin(f"❌{SERVER} | {key} | Error fetching trade configs: {e}")
        return []
...
def get_lot_size(config, instruments_df):
    ...
    """
    try:
        tradable_symbol_config = config['TRADABLE_SYMBOL'][0]
        if 'trading_symbol' in tradable_symbol_config and tradable_symbol_config['trading_symbol'] and tradable_symbol_config['exchange']:
            OPTION_SYMBOL = tradable_symbol_config['symbol_name']
            SEGMENT = tradable_symbol_config['exchange'] + "-OPT"
        ...
        print(f"❌{SERVER} | {config['KEY']} | Error getting lot size: {e}")
        logging.error(f"❌{SERVER} | {config['KEY']} | Error getting lot size: {e}")
        return None




# def get_robust_optimal_option_old(signal, spot, instruments_df, config, user):
#     """
#     Robust option selector:
#       - BUY searches CE
#       - SELL searches PE
#       - spot is rounded to nearest 50
#       - expiry is selected from config['EXPIRY']
#       - selects the best option by strike proximity and liquidity
#     """
#     from kitefunction import get_entire_quote

#     signal_upper = signal.upper()
#     if signal_upper not in ("BUY", "SELL"):
#         raise ValueError(f"Unsupported signal: {signal}. Expected BUY or SELL.")

#     opt_type = "CE" if signal_upper == "BUY" else "PE"
#     rounded_spot = int(round(spot / 50.0) * 50)

#     print(f"{config['KEY']} | Robust search for {opt_type} around spot {spot} -> {rounded_spot}")
#     logging.info(f"{config['KEY']} | Robust search for {opt_type} around spot {spot} -> {rounded_spot}")
...
#     if target_options.empty:
#         print(f"❌ {SERVER} | {config['KEY']} | No {opt_type} options found for expiry {config['EXPIRY']}")
#         logging.error(f"{SERVER} | {config['KEY']} | No {opt_type} options found for expiry {config['EXPIRY']}")
#         return None, None, None, None

#     target_options['dist'] = (target_options['strike'] - rounded_spot).abs()
#     target_options = target_options.sort_values(['dist', 'expiry']).head(80)

#     best_option = None
#     best_dist = float('inf')
#     best_liquidity = -1

#     for _, opt in target_options.iterrows():
...
#         except Exception as e:
#             logging.error(f"❌ {SERVER} | {config['KEY']} | Quote error for {opt_symbol}: {e}")
#             continue

#         dist = float(opt['dist'])
#         if best_option is None or dist < best_dist or (dist == best_dist and liquidity > best_liquidity):
#             best_dist = dist
#             best_liquidity = liquidity
#             best_option = (opt_symbol, int(opt['strike']), opt['expiry'].strftime('%Y-%m-%d'), ltp)

#         if best_dist == 0 and best_liquidity > 0:
#             break

#     if best_option:
#         opt_symbol, opt_strike, opt_expiry, opt_ltp = best_option
#         print(f"✅ {config['KEY']} | Robust option found: {opt_symbol} | Strike: {opt_strike} | Expiry: {opt_expiry} | LTP: {opt_ltp}")
#         logging.info(f"{config['KEY']} | Robust option found: {opt_symbol} | Strike: {opt_strike} | Expiry: {opt_expiry} | LTP: {opt_ltp}")
#         return best_option

#     print(f"❌ {SERVER} | {config['KEY']} | No robust {opt_type} option matched")
#     logging.error(f"❌ {SERVER} | {config['KEY']} | No robust {opt_type} option matched")
#     return None, None, None, None


# def get_optimal_option_Static_for_Nifty(signal, spot, nearest_price, instruments_df, config, user):
#     from kitefunction import get_entire_quote
...
#                         break
#                 except Exception as e:
#                     print(f"❌ {SERVER} | {config['KEY']} | API Error on {opt_symbol}: {e}")
#                     logging.error(f"❌ {SERVER} | {config['KEY']} | API Error on {opt_symbol}: {e}")
#                 time.sleep(0.5)
...
#             return best_option
#         else:
#             print(f"❌ {SERVER} | {config['KEY']} | Rejected: {opt_symbol} price {opt_ltp} is too far from {nearest_price}")
#             logging.error(f"❌ {SERVER} | {config['KEY']} | Rejected: {opt_symbol} price {opt_ltp} is too far from {nearest_price}")

#     return None, None, None, None

def get_robust_optimal_option(signal, spot, instruments_df, config, user):
    """
    Robust option selector:
      - BUY searches CE
      - SELL searches PE
    ...
    from kitefunction import get_entire_quote
    tradable_symbol_config = config['TRADABLE_SYMBOL'][0]
    # OPTION_SYMBOL = tradable_symbol_config['trading_symbol']
    # SEGMENT = tradable_symbol_config['segment']
    if 'trading_symbol' in tradable_symbol_config and tradable_symbol_config['trading_symbol'] and tradable_symbol_config['exchange']:
        OPTION_SYMBOL = tradable_symbol_config['symbol_name']
        SEGMENT = tradable_symbol_config['exchange'] + "-OPT"
    ...
    if config['EXPIRY'] == "THIS_WEEK":
        days = 0
    elif config['EXPIRY'] == "NEXT_WEEK":
        days = 7
    elif config['EXPIRY'] == "NEXT_TO_NEXT_WEEK":
        days = 14
    else:
    ...
    if target_options.empty:
        print(f"❌ {config['KEY']} | No {opt_type} options found for expiry {config['EXPIRY']}")
        # logging.error(f"{config['KEY']} | No {opt_type} options found for expiry {config['EXPIRY']}")
        return None, None, None, None
    ...
    for _, opt in target_options.iterrows():
        ...
        try:
            ...
            # logging.error(f"{config['KEY']} | Quote error for {opt_symbol}: {e}")
            continue
    ...
    print(f"❌ {config['KEY']} | No robust {opt_type} option matched")
    # logging.error(f"{config['KEY']} | No robust {opt_type} option matched")
    return None, None, None, None
...
def get_hedge_option(signal, spot, strike, main_ltp, instruments_df, config, user):
    ...
    if df_filtered.empty:
        logging.error(f"❌ {SERVER} | {config['KEY']} | No hedge options found for strike {hedge_strike}")
        print(f"❌ {SERVER} | {config['KEY']} | No hedge options found for strike {hedge_strike}")
        return None, None, None, None
    ...
    if hedge_ltp is None:
        print(f"❌ {SERVER} | {config['KEY']} | Failed to fetch LTP for hedge {opt_symbol}")
        logging.error(f"❌ {SERVER} | {config['KEY']} | Failed to fetch LTP for hedge {opt_symbol}")
        return None, None, None, None

    # THE CORE FIX: Reject if hedge is more expensive than the main trade
    if hedge_ltp >= main_ltp:
        print(f"❌ {SERVER} | {config['KEY']} | Hedge Option {opt_symbol} hedge ltp ({hedge_ltp}) >= Main ltp ({main_ltp}). Strategy Inverted. Aborting.")
        logging.error(f"❌ {SERVER} | {config['KEY']} | Hedge Option {opt_symbol} hedge ltp ({hedge_ltp}) >= Main ltp ({main_ltp}). Strategy Inverted. Aborting.")
        return None, None, None, None

    print(f"✅ {SERVER} | {config['KEY']} | Hedge found: {opt_symbol} | LTP: {hedge_ltp}")
    logging.info(f"✅ {SERVER} | {config['KEY']} | Hedge found: {opt_symbol} | LTP: {hedge_ltp}")
    ...
    try:
        ...
        sql = """
            INSERT INTO open_trades (
                ...
                option_buy_price, entry_time, qty, interval, real_trade, 
                entry_reason, expiry_type, strategy, key, user_id, 
                hedge_option_symbol, hedge_strike, hedge_option_sell_price, 
        ...
        print(f"❌{SERVER} | {config['KEY']} | Error saving open position: {e}")
        logging.error(f"{SERVER} | {config['KEY']} | Error saving open position for {trade.get('OptionSymbol')}: {e}")
        send_telegram_message_admin(f"❌{SERVER} | {config['KEY']} | Error saving open position for {trade.get('OptionSymbol')}: {e}")

# def save_open_position_bkp_17_3_2026(trade, config, tradeGenie_id):
#     try:
#         logging.info(f"Saving open position: {trade['OptionSymbol']} in {config['INTERVAL']} interval")
#         conn = sqlite3.connect(DB_FILE)
#         c = conn.cursor()
#         sql = """
#             INSERT INTO open_trades (signal, spot_entry, option_symbol, strike, expiry, option_sell_price, entry_time, qty, interval, real_trade, entry_reason, expiry_type, strategy, key, user_id, hedge_option_symbol, hedge_strike, hedge_option_buy_price, hedge_qty, hedge_entry_time, trade_status)
#             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
#         """
#         params = (
...
#             trade.get("hedge_entry_time"), trade.get("trade_status")
#         )
...
def delete_open_position(symbol, config, trade, tradeGenie_id):
    try:
        ...
        print(f"❌ {SERVER} | {config['KEY']} | Error deleting open position for {symbol}: {e}")
        logging.error(f"{SERVER} | {config['KEY']} | Error deleting open position for {symbol}: {e}")
        send_telegram_message_admin(f"❌ {SERVER} | {config['KEY']} | Error deleting open position for {symbol}: {e}")
...
def load_open_position(config,key , user , tradeGenie_id):
    ...
    sql = """
        SELECT signal, spot_entry, option_symbol, strike, expiry, 
               option_buy_price, entry_time, qty, interval, real_trade, entry_reason, expiry_type, strategy, key, user_id, hedge_option_symbol, hedge_strike, hedge_option_sell_price, hedge_qty, hedge_entry_time
    ...
    position_data = {
        ...
        "hedge_strike": row[16],
        "hedge_option_sell_price": row[17],
        "hedge_qty": row[18],
    ...
    if config['HEDGE_TYPE'] != "NH":
        if position_data['RealTrade'] == "yes":
            ...
            if hedge_broker_qty >= position_data["hedge_qty"] and position_data["RealTrade"] == "yes":
                ...
                err_msg = f"❌ {SERVER} | {key}  | Broker qty {hedge_broker_qty} for {position_data['hedge_option_symbol']} is less than DB qty {position_data['hedge_qty']} — treating as closed."
    ...
    if position_data['RealTrade'] == "yes":    
        ...
        if broker_qty >= position_data["qty"]:
            ...
            err_msg = f"❌ {SERVER} | {key}  |  Broker qty {broker_qty} for {position_data['OptionSymbol']} is less than DB qty {position_data['qty']} — treating as closed."
            logging.error(err_msg)
            send_telegram_message_admin(err_msg)
            send_telegram_message(err_msg, user['telegram_chat_id'], user['telegram_token'])
            update_trade_config_on_failure(config['KEY'], err_msg, user)
...
def update_trade_status(trade, config, tradeGenie_id, new_status):
    try:
        ...
        print(f"❌ {SERVER} | {config['KEY']} | Error updating trade status for {trade['OptionSymbol']}: {e}")
        logging.error(f"{SERVER} | {config['KEY']} | Error updating trade status for {trade['OptionSymbol']}: {e}")
        send_telegram_message_admin(f"❌ {SERVER} | {config['KEY']} | Error updating trade status for {trade['OptionSymbol']}: {e}")
...
def record_trade(trade, config, tradeGenie_id):
    ...
    try:
        ...
        print(f"❌ {SERVER} | {config['KEY']} | Error recording trade: {e}")
        logging.error(f"{SERVER} | {config['KEY']} | Error recording completed trade: {e}")
        send_telegram_message_admin(f"❌ {SERVER} | {config['KEY']} | Error recording completed trade: {e}")
...
def get_next_expiry_optimal_option(signal, last_expiry, price, nearest_price, instruments_df, config, user):
    ...
    try:
        ...
        print(f"❌{SERVER} |  {config['KEY']} | Error in get_next_expiry_optimal_option: {e}")
        logging.error(f"❌ {SERVER} | {config['KEY']} | Error in get_next_expiry_optimal_option: {e}")
        send_telegram_message_admin(f"❌ {SERVER} | {config['KEY']} | Error in get_next_expiry_optimal_option: {e}")
        return None, None, None, None
...
def close_position_and_no_new_trade(trade, position, close, ts, config, user, key, reason="Nothing"):
    ...
    if not is_valid_trade_data(exit_qty, avg_price, hedge_avg_price, hedge_required):
        err_msg = f"⚠️ {SERVER} | {key} | FAILED EXIT: Qty: ({exit_qty}) or Price: ({avg_price}) is 0 for {trade['OptionSymbol']}. DB NOT updated."
        logging.error(err_msg)
        send_telegram_message_admin(err_msg)
        update_trade_config_on_failure(config['KEY'], err_msg, user)
        return    
...
def who_tried(user):
    ...
    if profile is None:
        print(f"❌{SERVER} | `{user['user']} Profile not found. Exiting.")
        logging.error(f"❌{SERVER} | {user['user']}  Profile not found. Exiting.")
        send_telegram_message(f"❌{SERVER} | {user['user']}  Profile not found. Exiting.",user['telegram_chat_id'], user['telegram_token'])
        return
    else:
...
def check_monthly_stoploss_hit(user, config):
    ...
    for sl in monthly_sls:
        ...
        if monthly_stopLoss is not None and total_pnl <= -abs(monthly_stopLoss):
            ...
            logging.info(f"🚫 {config['KEY']} | {user['user']} | MONTHLY_STOPLOSS limit reached ({total_pnl} <= -{monthly_stopLoss}). No new trades allowed. Skipping till next month.")
            return True

    print(f"✅ {config['KEY']} | {user['user']} | MONTHLY_STOPLOSS limits not reached (PnL: {total_pnl}).")
    logging.info(f"✅ {config['KEY']} | {user['user']} | MONTHLY_STOPLOSS limits not reached (PnL: {total_pnl}).")
    return False
...
def check_trade_stoploss_hit(user, trade, config):
    ...
    if Stoplosses is None or len(Stoplosses) == 0:
        ...
        logging.info(f"ℹ️ {config['KEY']} | No Stoplosses found for trade check.")
        return False

    trade_sls = [sl for sl in Stoplosses if sl['SCOPE'] == 'PER_TRADE']
    if not trade_sls:
        print(f"ℹ️ {config['KEY']} | No PER_TRADE Stoplosses found.")
        logging.info(f"ℹ️ {config['KEY']} | No PER_TRADE Stoplosses found.")

        return False

    print(f"ℹ️ Checking TRADE_STOPLOSS for {config['KEY']} interval on trade {trade['OptionSymbol']}")
    logging.info(f"ℹ️ Checking TRADE_STOPLOSS for {config['KEY']} interval on trade {trade['OptionSymbol']}")
    ...
    total_pnl = total_pnl * trade["qty"]  # Scale by quantity for accurate PnL
    ...
    for sl in trade_sls:
        ...
        if sl['SL_TYPE'] == 'STATIC':
            ...
            stopLoss_percentage = sl['SL_VALUE']
            total_buy_price = trade["OptionBuyPrice"] * trade["qty"]
...
def check_login_success(user,password):
    try:
        ...
        print(f"❌{SERVER} | {user} Error checking login: {e}")
        logging.error(f"❌{SERVER} | {user} | Error checking login: {e}")
        return False, None
...
def update_trade_config_on_failure(config_key, kill_reason, user):
    ...
    try:
        ...
        send_telegram_message_admin(err_msg)
        logging.critical(f"{SERVER} | {user['user']} | Database updated: {config_key} is now BLOCKED. Reason: {kill_reason}")
        
    except Exception as e:
        logging.error(f"{SERVER} | {user['user']} | {config_key} | Failed to update trade_config on failure: {e}")
...
def validate_trade_prices(main_symbol, hedge_symbol, config, user):
    ...
    if hedge_ltp >= main_ltp:
        logging.error(f"❌{SERVER} | {config['KEY']} |  REJECTED: Hedge ({hedge_ltp}) >= Main ({main_ltp}). Invalid Credit.")
        return False

    # RULE 2: Hard Abort (25 Points)
    if price_diff > 25.0:
        logging.critical(f"☢️{SERVER} | {config['KEY']} | ABORT: Main {main_ltp} is {price_diff}pts from Target {target_ltp}.")
        return False

    # RULE 3: Soft Tolerance (10 Points)
    if price_diff > 10.0:
        logging.warning(f"⚠️{SERVER} | {config['KEY']} | CAUTION: Main {main_ltp} moved {price_diff}pts from Target.")
